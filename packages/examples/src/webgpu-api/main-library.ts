...
import { cubeNormals, cubePositions } from './cube-data';
...
const pipeline = device.createRenderPipeline({
    ...
    vertex: {
        module: shaderModule,
        entryPoint: 'vs',
        buffers: Vertex.bufferLayout.nonInterleaved,
    },
    fragment: {
        module: shaderModule,
        entryPoint: 'fs',
...
const vertex = Vertex.createNonInterleaved(device, {
...
const frame = (t: number) => {
    ...
    for (const attr of vertex.attributes) {
        pass.setVertexBuffer(attr.slot, attr.buffer);
    }
