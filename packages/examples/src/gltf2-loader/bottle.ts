...
const run = async () => {
    const Loader = Gltf2Loader.createLoader({
        parserOptions: {
            resolveImageUrl: (uri) => `./WaterBottle/glTF/${uri}`,
            resolveBufferUrl: (uri) => `./WaterBottle/glTF/${uri}`,
        },
    });

    const result = await Loader.load('./WaterBottle/glTF/WaterBottle.gltf');
    console.log(result);
    ...
    for (let mtidx = 0; mtidx < result.materialTypes.length; mtidx++) {
        ...
        for (let plidx = 0; plidx < result.primitiveLayouts.length; plidx++) {
            ...
            console.log({ materialType, VertexLayout });
    ...
    const view = Mat4x4.createLookAt(Vec3.create(0.5, 0, 0.5), Vec3.zero(), Vec3.up());
