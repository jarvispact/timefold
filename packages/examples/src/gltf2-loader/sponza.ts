...
//     vertices: { slot: number; buffer: GPUBuffer };
//     // uv: { slot: number; buffer: GPUBuffer };
//     // normal: { slot: number; buffer: GPUBuffer };
//     // tangent: { slot: number; buffer: GPUBuffer };
...
const run = async () => {
    const gltfJson = await fetch('./sponza/glTF/Sponza.gltf').then((res) => res.text());

    const parser = Gltf2Parser.createParser({
        resolveBufferUrl: (uri) => `./sponza/glTF/${uri}`,
        resolveImageUrl: (uri) => `./sponza/glTF/${uri}`,
    });
    ...
    for (let mtidx = 0; mtidx < result.materialTypes.length; mtidx++) {
        ...
        for (let plidx = 0; plidx < result.primitiveLayouts.length; plidx++) {
            ...
            console.log({ primitiveLayout });

            // Incompatible - skip
            if (
                !primitiveLayout.attributes.NORMAL ||
                !primitiveLayout.attributes.TEXCOORD_0 ||
                !primitiveLayout.attributes.TANGENT
            ) {
                continue;
            }

            const NonInterlevedVertexLayout = WebgpuUtils.createVertexBufferLayout('non-interleaved', {
                ...
                tangent: primitiveLayout.attributes.TANGENT,
            });

            const InterleavedVertexLayout = WebgpuUtils.createVertexBufferLayout('interleaved', {
                position: primitiveLayout.attributes.POSITION,
                uv: primitiveLayout.attributes.TEXCOORD_0,
                normal: primitiveLayout.attributes.NORMAL,
                tangent: primitiveLayout.attributes.TANGENT,
            ...
            const VertexLayout =
                primitiveLayout.type === 'interleaved' ? InterleavedVertexLayout : NonInterlevedVertexLayout;
            ...
            const primitives = result.primitives
                .map((primitive, pi) => {
                    const layout = result.primitiveLayouts[primitive.primitiveLayout];

                    // console.log({ layout, primitive });
                    ...
                    const buffers: RenderTreeBuffers =
                        layout.type === 'non-interleaved'
                            ? {
                                  type: 'non-interleaved',
                                  position: NonInterlevedVertexLayout.createBuffer(
                                      device,
                                      'position',
                                      primitive.attributes.POSITION,
                                  ),
                                  normal: NonInterlevedVertexLayout.createBuffer(
                                      device,
                                      'normal',
                                      primitive.attributes.NORMAL,
                                  ),
                                  uv: NonInterlevedVertexLayout.createBuffer(
                                      device,
                                      'uv',
                                      primitive.attributes.TEXCOORD_0,
                                  ),
                                  tangent: primitive.attributes.TANGENT
                                      ? NonInterlevedVertexLayout.createBuffer(
                                            device,
                                            'tangent',
                            ...
                            : {
                                  type: 'interleaved',
                                  vertices: InterleavedVertexLayout.createBuffer(device, primitive.attributes.vertices),
                                  index,
                              };
                    ...
                    // const tangent = VertexLayout.createBuffer(device, 'tangent', primitive.attributes.TANGENT);

                    // const vertices = VertexLayout.createBuffer(device, primitive.attributes.vertices);

                    return {
                        primitive,
                        buffers,
                        meshes: (result.meshesForPrimitive[pi] ?? [primitive.mesh]).map((m) => {
    ...
    const view = Mat4x4.createLookAt(Vec3.create(0, 3000, 5000), Vec3.zero(), Vec3.up());
    ...
    const render = () => {
        ...
        for (const { pipeline, sceneBindGroup, primitives } of renderTree.pipelines) {
            ...
            for (const primitive of primitives) {
                if (primitive.buffers.type === 'non-interleaved') {
                    ...
                    if (primitive.buffers.tangent) {
                        pass.setVertexBuffer(primitive.buffers.tangent.slot, primitive.buffers.tangent.buffer);
                    }
                } else {
                    pass.setVertexBuffer(primitive.buffers.vertices.slot, primitive.buffers.vertices.buffer);
                }
                ...
                for (const mesh of primitive.meshes) {
                    ...
                    } else if (primitive.buffers.type === 'non-interleaved') {
