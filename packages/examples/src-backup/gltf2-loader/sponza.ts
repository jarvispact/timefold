...
const run = async () => {
    ...
    // const result = await Loader.load('./Sponza/glTF/Sponza.gltf');

    const Loader = Gltf2Loader.createLoader({
        parserOptions: {
            resolveBufferUrl: (uri) => `./sponza-optimized/${uri}`,
            resolveImageUrl: (uri) => `./sponza-optimized/${uri}`,
        },
    });

    const result = await Loader.load('./sponza-optimized/Sponza.gltf');
    console.log(result);
    ...
    for (let mtidx = 0; mtidx < result.materialTypes.length; mtidx++) {
        ...
        for (let plidx = 0; plidx < result.primitiveLayouts.length; plidx++) {
            ...
            console.log(VertexLayout.wgsl);
            console.log(VertexLayout.layout);
    ...
    const view = Mat4x4.createLookAt(Vec3.create(2, 5, 1), Vec3.create(1, 4, -0.5), Vec3.up());
    ...
    Vec3.normalization(scene.views.sun_direction, Vec3.create(10, 10, 10));
